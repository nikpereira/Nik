...
async function getAllAgents() {
  const r = await db.execute(`SELECT id, name, email, role, is_active, created_at FROM agents WHERE role='agent' ORDER BY created_at DESC`);
  return r.rows;
}
...
async function findAgentByEmail(email) {
  const r = await db.execute({ sql: `SELECT * FROM agents WHERE email=? AND is_active=1 LIMIT 1`, args: [email] });
